...
Im Folgenden wollen wir uns speziell auf die reellen Zahlen konzentrieren. Ein Intervall ist eine Teilmenge von $\mathbb{R}$, die durch die Angabe begrenzender Elemente gebildet wird. Es gibt zwei Möglichkeiten, eine Intervallgrenze $a\in \mathbb{R}$ aufzufassen:
...
\begin{minipage}{0.5\textwidth}
        \centering
        \begin{tikzpicture}
            \draw[thick, -{latex}] (-1.7,0) -- (1.7,0)node[right]{$x$, Abszisse};
            \draw[thick, -{latex}] (0,-1.7) -- (0,1.7)node[left]{$y$, Ordinate};
            \node (A) at (0.8,0.8){\normalsize\uppercase\expandafter{\romannumeral 1\relax}};
            \node (A) at (-0.8,0.8){\normalsize\uppercase\expandafter{\romannumeral 2\relax}};
            \node (A) at (-0.8,-0.8){\normalsize\uppercase\expandafter{\romannumeral 3\relax}};
            \node (A) at (0.8,-0.8){\normalsize\uppercase\expandafter{\romannumeral 4\relax}};
        \end{tikzpicture}
\end{minipage}
...
\begin{figure}[htp]
    \centering
    \begin{tikzpicture}
        \begin{axis}[disabledatascaling, axis lines=middle, xtick, ytick, xlabel={$x$}, ylabel={$y$}, height=8cm, width=0.8\textwidth, xmin=-1, ymin=-.5, xmax=5.5, ymax=3.5]
            \draw[dashed] (0,.5) -- (5,.5);
...
\begin{wrapfigure}{r}{.6\textwidth}
        ...
        \vspace{-.5cm}
        \begin{tikzpicture}
            \begin{axis}[disabledatascaling, axis lines=middle, xtick, ytick, xlabel={$x$}, ylabel={$y$}, height=6cm, width=0.6\textwidth, ymax=2.5, ymin=-1.5, xmin=-1, xmax=3.5]
...
Von einem \emph{unterbestimmten} Gleichungssystem spricht man, wenn es mehr Unbekannte als Gleichungen gibt, bzw. Gleichungen linear abhängig sind. Schauen wir uns hierzu ein Beispiel an: 
\begin{subequations}
    \begin{align}
